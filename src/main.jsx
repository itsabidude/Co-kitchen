...
function CustomerLanding() {
  ...
  const [themeIndex]=useState(()=>Math.floor(Math.random()*5));
  ...
  const checkout=()=>{if(!name.trim()){setError('Enter your name, you silly Stark');setPage('home');return}setPage('checkout')};
...
function foodEmoji(name){
  ...
  if(n.includes('appam')) return '🥞';
  if(n.includes('puttu')) return '🍚';
  if(n.includes('pothichoru')) return '🍱';
  if(n.includes('kadala')) return '🫘';
  if(n.includes('chicken')) return '🍗';
  if(n.includes('stew')) return '🥘';
  return '🍛';
}
...
function CustomerFeedback({name,onBack}) {
  ...
  const submit=async()=>{
    ...
    try{
      if(cokitbaseReady){
        const {error}=await cokitbase.from('testimonials').insert({customer_name:name?.trim()||'Customer',rating:rating||5,message:suggestion.trim()});
        if(error)throw error;
      }
      setSubmitted(true);
    }catch(e){alert(e.message||'Could not submit feedback.');}
    finally{setSaving(false);}
  };
  if(submitted) return <div className="customer-flow feedback-page"><header className="site-header"><div className="customer-brand compact-customer-brand"><img className="brand-logo-image" src={COCO_LOGO} alt="CO-CO Kitchen" /></div></header><main className="customer-content centered"><span className="admin-eyebrow">THANK YOU</span><h1>Your feedback matters.</h1><p>Thank you for sharing your experience{name ? ', '+name : ''}.</p><p className="feedback-moderation-note">Your testimonial will appear publicly only after CO-CO Kitchen approves it.</p><button className="start-button" onClick={onBack}>RETURN TO PAYMENT STATUS <ArrowRight size={18}/></button></main></div>;
  return <div className="customer-flow feedback-page"><header className="site-header"><div className="customer-brand compact-customer-brand"><img className="brand-logo-image" src={COCO_LOGO} alt="CO-CO Kitchen" /></div></header><main className="customer-content centered"><div className="customer-hero"><span>YOUR EXPERIENCE</span><h1>Suggestions & Testimonials</h1><p>We would value your feedback on today's meal.</p></div><section className="feedback-card"><label>HOW WAS YOUR EXPERIENCE?</label><div className="rating-row" aria-label="Rating out of five">{[1,2,3,4,5].map(n=><button key={n} type="button" className={rating>=n?'rated':''} onClick={()=>setRating(n)} aria-label={'Rate '+n+' out of 5'}>★</button>)}</div><label>YOUR SUGGESTION OR TESTIMONIAL<textarea value={suggestion} onChange={e=>setSuggestion(e.target.value)} placeholder="Tell us what you enjoyed or what we could improve." rows="5"/></label><button className="start-button" disabled={saving||(!rating&&!suggestion.trim())} onClick={submit}>{saving?'SUBMITTING…':'SUBMIT FEEDBACK'} <ArrowRight size={18}/></button><button className="feedback-back-button" onClick={onBack}>BACK TO PAYMENT STATUS</button></section></main></div>;
}
...
function AdminOrderDetails({ order, onBack, onLogout }) {
    ...
    const refresh=async()=>{
      ...
      try{
        ...
        if(alive&&data){
          ...
          setPayment(data.payment_status||'PENDING');setStatus(data.order_status||'PLACED');
        }
      }catch(_e){}
    };
    refresh(); const unsub=subscribeToPortalChanges(refresh);
    return()=>{alive=false;unsub()};
  },[order.raw?.id]);
  const verified=payment==='VERIFIED';
  const verifyPayment=async()=>{
    if(!cokitbaseReady||!currentOrder.raw?.id){setPayment('VERIFIED');return}
    setSaving(true);
    try{const {data,error}=await cokitbase.rpc('verify_order_payment',{p_order_id:currentOrder.raw.id});if(error)throw error;setPayment(data?.payment_status||'VERIFIED');setLiveOrder(o=>({...o,total:o.total,raw:{...o.raw,payment_status:data?.payment_status||'VERIFIED'}}));}
    catch(err){alert(err.message||'Could not verify payment.')}finally{setSaving(false)}
  };
  ...
  const changeItem=async(item)=>{
    ...
    if(!cokitbaseReady){setEditing({});return}
    setSaving(true);
    try{const {data,error}=await cokitbase.rpc('update_order_item',{p_order_item_id:item.id,p_quantity:q});if(error)throw error;setLiveOrder(o=>({...o,total:'₹'+Number(data.total_amount||0).toFixed(0),raw:{...o.raw,order_items:(o.raw.order_items||[]).map(x=>x.id===item.id?{...x,quantity:q}:x),total_amount:data.total_amount}}));setEditing(e=>{const n={...e};delete n[item.id];return n});}
    catch(err){alert(err.message||'Could not change quantity.')}finally{setSaving(false)}
  };
  const deleteItem=async(item)=>{
    if(!confirm('Delete '+item.item_name+' from this order?'))return;
    if(!cokitbaseReady)return;
    setSaving(true);
    try{const {data,error}=await cokitbase.rpc('delete_order_item',{p_order_item_id:item.id});if(error)throw error;const next=(currentOrder.raw.order_items||[]).filter(x=>x.id!==item.id);setLiveOrder(o=>({...o,total:'₹'+Number(data.total_amount||0).toFixed(0),raw:{...o.raw,order_items:next,total_amount:data.total_amount}}));}
    catch(err){alert(err.message||'Could not delete item.')}finally{setSaving(false)}
  };
  return <div className="admin-shell dashboard-shell"><header className="admin-header"><div className="admin-brand"><img className="brand-logo-image" src={COCO_LOGO} alt="CO-CO Kitchen" /><div className="admin-brand-copy"><strong>CO-CO KITCHEN</strong><span>ADMINISTRATION</span></div></div><div className="admin-header-right"><span className="admin-date">{formatDate(new Date())}</span><button className="logout-button" onClick={onLogout}><LogOut size={16}/> LOG OUT</button></div></header>
    <main className="dashboard-content order-detail-page"><button className="back-dashboard" onClick={onBack}><ArrowLeft size={16}/> ALL ORDERS</button>
      ...
      <section className="detail-grid"><div className="dashboard-panel detail-panel"><div className="panel-heading"><div><span className="panel-kicker">CUSTOMER</span><h2>{currentOrder.name}</h2></div></div><div className="customer-details"><div><Phone size={16}/><span>{currentOrder.mobile||'Mobile not provided'}</span></div><div><MapPin size={16}/><span>{currentOrder.raw?.delivery_location||'Location not provided'}</span></div><div><CalendarDays size={16}/><span>{currentOrder.raw?.order_date||formatDate(new Date())}</span></div></div></div>
      <div className="dashboard-panel payment-detail-panel"><div className="panel-heading"><div><span className="panel-kicker">PAYMENT</span><h2>{currentOrder.total}</h2></div><b className={'pill '+payment.toLowerCase()}>{payment}</b></div><div className="payment-detail-body"><div className="payment-method"><CircleDollarSign size={18}/><div><strong>UPI PAYMENT</strong><span>{payment==='CUSTOMER_MARKED_PAID'?'Customer marked payment as completed':'Payment status from Cokitbase'}</span></div></div>{!verified&&<button className="verify-payment-button" disabled={saving} onClick={verifyPayment}><CheckCircle2 size={17}/> {saving?'VERIFYING…':'VERIFY PAYMENT'}</button>}{verified&&<div className="verified-note"><CheckCircle2 size={17}/> PAYMENT VERIFIED — CUSTOMER WILL SEE THIS UPDATE</div>}</div></div></section>
      <section className="dashboard-panel items-detail-panel"><div className="panel-heading"><div><span className="panel-kicker">ORDER SUMMARY</span><h2>Items Ordered</h2></div><span className="meal-label">EDIT QUANTITY / DELETE</span></div><div className="detail-items editable-detail-items">{items.length?items.map(i=><div className="editable-order-item" key={i.id}><div><strong>{i.item_name}</strong><small>₹{Number(i.unit_price).toFixed(0)} each</small></div><div className="order-item-actions"><input className="order-qty-input" type="number" min="1" value={editing[i.id]??i.quantity} onChange={e=>setEditing({...editing,[i.id]:e.target.value})}/><button className="item-save-button" disabled={saving} onClick={()=>changeItem(i)}>SAVE</button><button className="item-delete-button" disabled={saving} onClick={()=>deleteItem(i)}>DELETE</button></div></div>):<div><span>No items remain in this order.</span><strong>₹0</strong></div>}</div><div className="detail-total"><span>ORDER TOTAL</span><strong>{currentOrder.total}</strong></div></section>
...
function AdminOrders({ onBack, onOpenOrder, onLogout }) {
  ...
  const add=async()=>{if(!form.item_name.trim()||!Number(form.quantity)||form.unit_price==='')return alert('Enter item, quantity and price.');setSaving(true);try{const {error}=await cokitbase.from('expenses').insert({purchase_date:date,item_name:form.item_name.trim(),quantity:Number(form.quantity),unit:form.unit,unit_price:Number(form.unit_price),notes:form.notes.trim()});if(error)throw error;setForm({item_name:'',quantity:'',unit:'kg',unit_price:'',notes:''});load()}catch(e){alert(e.message||'Could not save purchase.')}finally{setSaving(false)}};
  const total=rows.reduce((s,x)=>s+Number(x.total_cost||0),0);
  return <AdminPageShell title="Purchases & Expenditure." eyebrow="COST CONTROL" subtitle="Record what you buy so food cost, break-even and margins can be tracked." onBack={onBack} onLogout={onLogout}><div className="tracker-date"><label>DATE<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label><strong>DAY SPEND ₹{total.toLocaleString('en-IN',{maximumFractionDigits:2})}</strong></div><section className="dashboard-panel"><div className="panel-heading"><div><span className="panel-kicker">ADD PURCHASE</span><h2>Items brought in</h2></div></div><div className="purchase-form"><label>ITEM NAME<input value={form.item_name} onChange={e=>setForm({...form,item_name:e.target.value})} placeholder="Rice, chicken, coconut…"/></label><label>QUANTITY<input type="number" min="0" step="0.001" value={form.quantity} onChange={e=>setForm({...form,quantity:e.target.value})}/></label><label>UNIT<select value={form.unit} onChange={e=>setForm({...form,unit:e.target.value})}><option>kg</option><option>L</option><option>pcs</option><option>pack</option><option>other</option></select></label><label>PRICE / UNIT (₹)<input type="number" min="0" step="0.01" value={form.unit_price} onChange={e=>setForm({...form,unit_price:e.target.value})}/></label><label>NOTES<input value={form.notes} onChange={e=>setForm({...form,notes:e.target.value})} placeholder="Supplier / bill note"/></label><button className="admin-primary" disabled={saving} onClick={add}>{saving?'SAVING…':'ADD PURCHASE'} <ArrowRight size={16}/></button></div></section><section className="dashboard-panel"><div className="panel-heading"><div><span className="panel-kicker">PURCHASE LOG</span><h2>{date} Purchases</h2></div></div><div className="expense-table">{rows.map(x=><div className="expense-row" key={x.id}><span><strong>{x.item_name}</strong><small>{x.quantity} {x.unit} × ₹{Number(x.unit_price).toFixed(2)}</small></span><b>₹{Number(x.total_cost).toFixed(2)}</b></div>)}{!rows.length&&<p className="panel-description">No purchases recorded for this date.</p>}</div></section></AdminPageShell>;
}

function AdminAnalytics({onBack,onLogout}){
  const [alertMobile,setAlertMobile]=useState(localStorage.getItem('cocoAlertMobile')||'');
  const [alertSaved,setAlertSaved]=useState(false);
  const [date,setDate]=useState(new Date().toISOString().slice(0,10)); const [orders,setOrders]=useState([]); const [expenses,setExpenses]=useState([]);
  const load=async()=>{if(!cokitbaseReady)return;const [o,e]=await Promise.all([cokitbase.from('orders').select('*,order_items(*)').eq('order_date',date),cokitbase.from('expenses').select('*').eq('purchase_date',date)]);setOrders(o.data||[]);setExpenses(e.data||[])};useEffect(()=>{load()},[date]);
  const revenue=orders.filter(x=>x.payment_status==='VERIFIED').reduce((s,x)=>s+Number(x.total_amount||0),0); const spend=expenses.reduce((s,x)=>s+Number(x.total_cost||0),0); const net=revenue-spend; const avg=orders.length?revenue/orders.length:0;
  const itemCounts={};orders.forEach(o=>(o.order_items||[]).forEach(i=>{itemCounts[i.item_name]=(itemCounts[i.item_name]||0)+i.quantity}));
  return <AdminPageShell title="Daily Business Tracker." eyebrow="OPERATIONS & DATA" subtitle="Revenue, purchases and orders in one place. This history can power forecasting later." onBack={onBack} onLogout={onLogout}><section className="dashboard-panel alert-settings-panel"><div className="panel-heading"><div><span className="panel-kicker">ORDER ALERTS</span><h2>New-order mobile alert</h2></div><span className="menu-state">SETTINGS</span></div><div className="alert-settings-body"><label>ALERT MOBILE NUMBER<input value={alertMobile} onChange={e=>setAlertMobile(e.target.value.replace(/\D/g,'').slice(0,10))} placeholder="10-digit mobile number" inputMode="numeric"/></label><button className="admin-primary" onClick={()=>{if(!/^\d{10}$/.test(alertMobile))return alert('Enter a valid 10-digit mobile number.');localStorage.setItem('cocoAlertMobile',alertMobile);setAlertSaved(true);setTimeout(()=>setAlertSaved(false),2500)}}>SAVE ALERT NUMBER</button><p>{alertSaved?'Alert number saved for the notification service.':'The number is stored here for SMS/WhatsApp order alerts. A messaging-provider connection is required for automatic mobile delivery.'}</p></div></section><div className="tracker-date"><label>TRACK DATE<input type="date" value={date} onChange={e=>setDate(e.target.value)}/></label></div><section className="analytics-grid"><StatCard icon={TrendingUp} label="VERIFIED REVENUE" value={'₹'+revenue.toLocaleString('en-IN')} detail={date}/><StatCard icon={CircleDollarSign} label="ITEMS BROUGHT" value={'₹'+spend.toLocaleString('en-IN')} detail="Recorded expenditure"/><StatCard icon={ClipboardList} label="ORDERS" value={orders.length} detail="All orders placed"/><StatCard icon={CircleDollarSign} label="REVENUE − SPEND" value={'₹'+net.toLocaleString('en-IN')} detail="Simple daily contribution"/><StatCard icon={TrendingUp} label="AVG REVENUE / ORDER" value={'₹'+avg.toFixed(0)} detail="Verified revenue ÷ all orders"/></section><section className="dashboard-panel"><div className="panel-heading"><div><span className="panel-kicker">ITEMS ORDERED</span><h2>Daily quantity demand</h2></div></div><div className="expense-table">{Object.entries(itemCounts).map(([name,q])=><div className="expense-row" key={name}><span><strong>{name}</strong><small>Customer orders</small></span><b>{q} units</b></div>)}{!Object.keys(itemCounts).length&&<p className="panel-description">No order items for this date.</p>}</div></section></AdminPageShell>;
}

function AdminTestimonials({onBack,onLogout}){
  const [rows,setRows]=useState([]); const load=async()=>{if(!cokitbaseReady)return;const {data}=await cokitbase.from('testimonials').select('*').order('created_at',{ascending:false});setRows(data||[])};useEffect(()=>{load();const unsub=subscribeToPortalChanges(load);return unsub},[]);
  const update=async(id,patch)=>{const {error}=await cokitbase.from('testimonials').update({...patch,updated_at:new Date().toISOString()}).eq('id',id);if(error)alert(error.message);else load()};
  const remove=async(id)=>{if(!confirm('Delete this testimonial?'))return;const {error}=await cokitbase.from('testimonials').delete().eq('id',id);if(error)alert(error.message);else load()};
  return <AdminPageShell title="Customer Voices." eyebrow="TESTIMONIALS" subtitle="Approve the feedback you want future customers to see on the home page." onBack={onBack} onLogout={onLogout}><section className="dashboard-panel testimonials-admin-panel">{rows.map(t=><article className="admin-testimonial-card" key={t.id}><div><div className="bubble-stars">{'★'.repeat(t.rating)}</div><h3>{t.customer_name}</h3><p>“{t.message}”</p></div><div className="testimonial-actions"><button className={t.is_approved?'toggle-on':''} onClick={()=>update(t.id,{is_approved:!t.is_approved})}>{t.is_approved?'APPROVED':'APPROVE'}</button><button className={t.is_featured?'toggle-on':''} disabled={!t.is_approved} onClick={()=>update(t.id,{is_featured:!t.is_featured})}>{t.is_featured?'FEATURED':'FEATURE ON HOME'}</button><button className="item-delete-button" onClick={()=>remove(t.id)}>DELETE</button></div></article>)}{!rows.length&&<p className="panel-description">No testimonials submitted yet.</p>}</section></AdminPageShell>;
}

function AdminPageShell({title,eyebrow,subtitle,onBack,onLogout,children}){
  return <div className="admin-shell dashboard-shell"><header className="admin-header"><div className="admin-brand"><img className="brand-logo-image" src={COCO_LOGO} alt="CO-CO Kitchen"/><div className="admin-brand-copy"><strong>CO-CO KITCHEN</strong><span>ADMINISTRATION</span></div></div><div className="admin-header-right"><span className="admin-date">{formatDate(new Date())}</span><button className="logout-button" onClick={onLogout}><LogOut size={16}/> LOG OUT</button></div></header><main className="dashboard-content admin-subpage"><button className="back-dashboard" onClick={onBack}><ArrowLeft size={16}/> DASHBOARD</button><section className="orders-page-intro"><div><span className="admin-eyebrow">{eyebrow}</span><h1>{title}</h1><p>{subtitle}</p></div></section>{children}</main></div>;
}

function AdminDashboard({onOrders,onMenu,onExpenses,onAnalytics,onTestimonials,onLogout}){
...
function AdminApp() {
  ...
  if(view==='expenses') return <AdminExpenses onBack={()=>setView('dashboard')} onLogout={logout}/>;
  if(view==='analytics') return <AdminAnalytics onBack={()=>setView('dashboard')} onLogout={logout}/>;
  if(view==='testimonials') return <AdminTestimonials onBack={()=>setView('dashboard')} onLogout={logout}/>;
